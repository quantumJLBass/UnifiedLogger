...
class UnifiedLogger:
    ...
    def display_toast(self, message: str, level: str = "info"):
        ...
        Toast(self.root, title="ttkbootstrap toast message",  message=message,duration=3000, bootstyle=boot_style,alert=False, icon=icon_name, iconfont=None, position= (100, 100, 'ne')).show_toast()
    ...
    def get_icon_with_matching_height(self, icon_name, text_label):
        # Get the font size of the text label
        font_size = text_label.cget("font")[-1]

        # Create the icon image with the height matching the font size
        icon_image = icon_to_image(icon_name, fill="#177255", scale_to_height=font_size)
